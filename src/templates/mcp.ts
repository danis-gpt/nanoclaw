...
export function readPluginMcp(pluginDir: string): { servers: Record<string, McpServerConfig>; report: string[] } {
  ...
  try {
    ...
    // eslint-disable-next-line no-catch-all/no-catch-all -- malformed plugin JSON explicitly disables only this optional component
  } catch {
...
function readServerEntry(name: string, entry: unknown, report: string[]): McpServerConfig | string {
  ...
  try {
    ...
    // eslint-disable-next-line no-catch-all/no-catch-all -- validation failures become a per-server skip reason
  } catch (err) {
