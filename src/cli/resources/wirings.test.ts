...
describe('wirings — exact thread filter', () => {
  it('creates a wiring with --thread-filter', async () => {
    ...
    expect((await getMessagingGroupAgent(row.id as string))!.thread_filter).toBe('telegram:-100123:101');
  });
  ...
  it('rejects a conflicting thread filter when the wiring already exists', async () => {
    ...
    expect((await getMessagingGroupAgent(existing.id as string))!.thread_filter).toBeNull();
  });
