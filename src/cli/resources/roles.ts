...
function hasExactRole(userId: string, role: UserRoleKind, groupId: string | null): boolean {
  return getUserRoles(userId).some((row) => row.role === role && row.agent_group_id === groupId);
}
  ...
  customOperations: {
    grant: {
      ...
      handler: async (args) => {
        ...
        if (!hasExactRole(userId, role, groupId)) {
          grantRole({
            user_id: userId,
            role,
    ...
    revoke: {
      ...
      handler: async (args) => {
        ...
        revokeRole(userId, role, groupId);
        if (hasExactRole(userId, role, groupId)) {
