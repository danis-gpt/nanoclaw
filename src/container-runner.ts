...
async function spawnContainer(session: Session): Promise<void> {
  ...
  // Artha's native Codex provider authenticates from its isolated mounted
  // CODEX_HOME and talks to the provider directly. Routing that provider
  // through OneCLI would add a second, incompatible credential path and make
  // a healthy native Codex session depend on an unrelated gateway.
  const gateway: GatewayContribution =
    provider === 'codex'
      ? {}
      : await getGatewayProvider().contribute({
