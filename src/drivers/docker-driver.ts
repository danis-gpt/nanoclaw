...
export class DockerSessionDriver implements SessionDriver {
  ...
  constructor(private readonly opts: DockerDriverOptions) {
    this.#cli = opts.cli ?? realCli(CONTAINER_RUNTIME_BIN);
    this.#policy = opts;
  }
...
export function userArgs(spec: SessionSpec): string[] {
  ...
  const args: string[] = [];
  // Rootless Podman maps the invoking host user to container root by default.
  // Without keep-id, --user <host uid> resolves to a subordinate uid and can no
  // longer read or write that host user's bind mounts. Docker has no such flag.
  if (CONTAINER_RUNTIME_BIN === 'podman') args.push('--userns', 'keep-id');
  args.push('--user', `${spec.runAs.uid}:${spec.runAs.gid}`);
  return args;
}
