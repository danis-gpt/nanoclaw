...
describe('router — per-wiring thread policy', () => {
  ...
  it('routes an exact forum topic only to the wiring with the matching thread_filter', async () => {
    ...
    await getDb().run(
      'UPDATE messaging_group_agents SET thread_filter = ? WHERE id = ?',
      'telegram:-100123:102',
      'mga-tp-2',
    );
    ...
    await withThreadedAdapter(async () => {
      ...
      expect(await getSessionsByAgentGroup('ag-tp')).toHaveLength(0);
      expect(await getSessionsByAgentGroup('ag-tp-2')).toHaveLength(1);
    });
  });
  ...
  it('preserves existing fanout behavior when thread_filter is null', async () => {
    ...
    await withThreadedAdapter(async () => {
      ...
      expect(await getSessionsByAgentGroup('ag-tp')).toHaveLength(1);
      expect(await getSessionsByAgentGroup('ag-tp-2')).toHaveLength(1);
    });
  });
