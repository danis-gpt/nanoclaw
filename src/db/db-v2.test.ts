...
describe('migrations', () => {
  ...
  it('adds a nullable exact thread filter and its lookup index', async () => {
    const db = await initSqliteTestDb();
    await runMigrations(db);
...
describe('messaging group agents', () => {
  ...
  it('persists an exact thread filter on a wiring', async () => {
    await createMessagingGroupAgent({ ...mga(), thread_filter: 'telegram:-100123:101' });
    expect((await getMessagingGroupAgent('mga-1'))!.thread_filter).toBe('telegram:-100123:101');
  });
