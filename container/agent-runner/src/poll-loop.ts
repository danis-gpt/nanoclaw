...
export async function processQuery(
  ...
  /** Artha container idle window. A positive value closes completed turns so
   * the outer loop can observe inactivity and exit cleanly. */
  idleTimeoutMs = 0,
    ...
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        if (event.text || event.files?.length) {
          ...
          const { sent, hasUnwrapped, taskBlocks, resultBlocks } = await dispatchResultText(resultText, routing, {
            midTurnSent,
            files: event.files,
          ...
          // second run summary.
          if (routing.taskRun && !taskBlockNudged && event.text) await autoAppendTaskLog(event.text);
          if (resultBlocks === 0 && event.isError === true && event.text && !routing.taskRun) {
...
export interface ResultDispatchOptions {
  /** Files generated by providers such as native Codex during this turn. */
  files?: ProviderFile[];
  /**
...
export async function dispatchResultText(
  ...
  while ((match = MESSAGE_RE.exec(text)) !== null) {
    ...
    const files = filesSent ? [] : (options?.files ?? []);
    await sendToDestination(dest, body, routing, files);
  ...
  // A generated binary has no reliable retry path: the bytes exist only on
  // this provider event. If no wrapped destination consumed it, attach it to
  // the originating conversation while keeping bare text delivery strict.
  if (!filesSent && (options?.files?.length ?? 0) > 0 && routing.channelType && routing.platformId) {
    const id = generateId();
    const filenames = writeOutboxFiles(id, options?.files ?? []);
    await writeMessageOut({
      id,
      in_reply_to: routing.inReplyTo,
      kind: 'chat',
